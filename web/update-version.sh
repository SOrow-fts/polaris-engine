...
$SED -e "s|.*STABLE-VERSION-AND-DATE.*|<!-- STABLE-VERSION-AND-DATE -->安定版 $VERSION \($DATE\):<br>|" \
     -e "s|.*STABLE-EXE.*|<!-- LATEST-EXE --><a href=\"/dl/polaris-engine-$VERSION_DIGIT.exe\">Windows版 Polaris Engine</a><br>|" \
     -e "s|.*STABLE-DMG.*|<!-- LATEST-DMG --><a href=\"/dl/polaris-engine-$VERSION_DIGIT.dmg\">Mac版 Polaris Engine</a><br>|" \
 > tmp
...
$SED -e "s|.*STABLE-VERSION-AND-DATE.*|<!-- STABLE-VERSION-AND-DATE -->Stable $VERSION \($DATE\):<br>|" \
     -e "s|.*STABLE-EXE.*|<!-- STABLE-EXE --><a href=\"/dl/polaris-engine-$VERSION_DIGIT.exe\">Download Polaris Engine for Windows</a><br>|" \
     -e "s|.*STABLE-DMG.*|<!-- STABLE-DMG --><a href=\"/dl/polaris-engine-$VERSION_DIGIT.dmg\">Download Polaris Engine for Mac</a><br>|" \
  > tmp
cat tmp | awk '/.*DOCTYPE html.*/,/.*LATEST-RELEASE-NOTE.*/' > before
cat tmp | awk '/.*LATEST-RELEASE-NOTE.*/,/\<\/html\>/' | tail -n +2 > after
cat before > new
